...
export default function ListaIzvodjacaAdminPage() {
  ...
  function izracunajProsek(
    ocene: OcenaMuzickiIzvodjacType[] | null | undefined
  ) {
  ...
  const handleOdobri = (index: number) => {
    ...
    axios
      .put(ApiConfig.BASE_URL + `/Administrator/OdobriNalog/${idIzvodjaca}`)
      .then((response) => {
  ...
  const handleOdbij = (index: number) => {
    ...
    axios
      .put(ApiConfig.BASE_URL + `/Administrator/OdbijNalog/${idIzvodjaca}`)
      .then((response) => {
